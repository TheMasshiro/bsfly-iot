...
router.post("/register", requireAuth, deviceLimiter, validateBody(registerSchema), async (req, res) => {
  try {
    ...
    const drawerNames = ["Drawer 1", "Drawer 2"];
...
router.patch("/:deviceId/control-mode", requireAuth, async (req, res) => {
  try {
    const userId = req.userId;
    ...
    const isMember = device.members.some((member) => member.userId === userId);
    if (!isMember) {
    ...
    try {
      const topic = `devices/${device.macAddress}/actuators/mode/control`;
      publishMqtt(topic, { state: mode }, { retain: true });
    } catch (error) {
      console.error("Failed to publish control mode update", error);
    }

    res.json(serializeDevice(device));
  } catch (error) {
    res.status(500).json({ error: "Failed to update control mode" });
  }
});
...
router.post("/:deviceId/heartbeat", async (req, res) => {
  try {
    ...
    res.json({ status: "online", lastSeen: device.lastSeen, ipAddress: device.ipAddress });
  } catch (error) {
...
router.get("/:deviceId/api-key", requireAuth, async (req, res) => {
  try {
    ...
    res.json({ apiKey: maskKey(device.apiKey), note: 'API key is masked. Regenerate the key to receive a new value.' });
  } catch (error) {
...
router.post("/:deviceId/regenerate-api-key", requireAuth, async (req, res) => {
  try {
    ...
    res.json({ apiKey: maskKey(device.apiKey), note: 'API key regenerated and masked in response.' });
  } catch (error) {
...
const HEARTBEAT_TIMEOUT = 15000; // 3x heartbeat interval for reliability
setInterval(async () => {
  try {
